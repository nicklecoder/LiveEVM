 ...
 *
 *********************************************************/
Amplifier::Amplifier(int tid, cv::Mat* pSrc, cv::Mat* pDst) {
...
void Amplifier::getNewFrames() {
    bool filter = Amplifier::mParams->getFilter();
	for (int i = 0; i < ((3 * BUF_SIZE) / 4); i++) {
        cv::Mat frame;
        this->mSrc[this->mCpyCursor].convertTo(frame, CV_64F);
		...
		frame.copyTo(this->mClean[this->mCpyCursor]);
...
void Amplifier::getAllFrames() {
	...
	for (int i = 0; i < BUF_SIZE; i++) {
        cv::Mat frame;
        this->mSrc[i].convertTo(frame, CV_64F);
...
void Amplifier::putNewFrames() {
    ...
    int rows = this->mSrc[idx].rows;
    int width = this->mSrc[idx].cols * this->mSrc[idx].channels();
	...
	for (int i = ((3 * BUF_SIZE) / 4); i < BUF_SIZE; i++) {
        ...
        cv::Mat tmp;
        this->mSrc[idx].convertTo(tmp, typeFrom);
